import type { FillComplete, FillPartial, Holding, StrategyRun } from "@ai-trading/domain";
...
export const METRICS_WINDOWS = ["all", "1h", "24h", "7d"] as const;
...
const toPositionKey = (venue: string, symbol: string): string => `${venue}:${symbol.trim().toUpperCase()}`;

const sortFills = (fills: Array<FillPartial | FillComplete>): FillComplete[] => {
  return fills
    .filter((fill): fill is FillComplete => fill.type === "FillComplete")
    .slice()
    .sort((left, right) => {
...
const applyMetricsWindow = (fills: FillComplete[], window: MetricsWindow): FillComplete[] => {
...
const buildHoldingsFromFills = (
  fills: FillComplete[],
  marksByKey: Map<string, number | undefined>,
): Holding[] => {
  ...
  for (const fill of fills) {
    ...
    if (fill.payload.side === "buy") {
      ...
      const weightedCost = roundToScale(
        existing.quantity * existing.averageCost + fill.payload.quantity * fill.payload.averagePrice,
        8,
      );
      positions.set(key, {
...
const computeRealizedStats = (fills: FillComplete[]): { realizedPnl: number; closedTrades: ClosedTradeStat[]; wins: number } => {
  const positions = new Map<string, PositionAccumulator>();
  const closedTrades: ClosedTradeStat[] = [];
  let realizedPnl = 0;
  let wins = 0;
...
const computeMaxDrawdown = (closedTrades: ClosedTradeStat[]): number => {
  let cumulative = 0;
  let peak = 0;
  let maxDrawdown = 0;
...
const computeSharpeLikeRatio = (
  closedTrades: ClosedTradeStat[],
  defaultDenominator: number,
  usePerTradeBasis: boolean,
): number => {
...
const computeStrategyCapitalBaseline = (fills: FillComplete[]): number => {
  const positions = new Map<string, PositionAccumulator>();
  let maxGrossCost = 0;
...
const computeUnrealizedAndExposure = (
  holdings: Holding[],
): { unrealizedPnl: number; grossExposure: number; warnings: MetricsWarning[] } => {
  let unrealizedPnl = 0;
  let grossExposure = 0;
  const missingMarkSymbols: string[] = [];
  ...
  for (const holding of holdings) {
    ...
    const marketPrice = holding.marketPrice as number;
    unrealizedPnl = roundToScale(
      unrealizedPnl + (marketPrice - holding.averageCost) * holding.quantity,
      8,
    );
...
const timestampInRange = (value: string, start: string, end: string): boolean => {
  return value >= start && value <= end;
};

export const filterFillsForStrategyRun = (
...
export const computeAccountAnalytics = (
  fills: Array<FillPartial | FillComplete>,
  holdings: Holding[],
  totalEquity: number,
  ...
  options?: {
    ...
    usePerTradeSharpeBasis?: boolean;
  },
  ...
  return {
    metrics: {
      ...
      winRate:
        realizedStats.closedTrades.length > 0
      ...
      sharpeLikeRatio: computeSharpeLikeRatio(
        realizedStats.closedTrades,
        roundedEquity,
        options?.usePerTradeSharpeBasis ?? false,
      ),
    },
...
export const computeStrategyAnalytics = (
  fills: Array<FillPartial | FillComplete>,
  ...
  const strategyFills = filterFillsForStrategyRun(fills, run);
  const marksByKey = new Map<string, number | undefined>();
  ...
  return computeAccountAnalytics(strategyFills, strategyHoldings, totalEquity, window, {
