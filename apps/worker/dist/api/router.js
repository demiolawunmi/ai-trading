...
const runtime_1 = require("../strategy/runtime");
const analytics_1 = require("../metrics/analytics");
...
const sendBadRequest = (res, code, message) => {
    res.status(400).json({
        error: {
            code,
            message,
...
const isVenue = (value) => {
    return typeof value === "string" && domain_1.VENUES.includes(value);
};
const parseQuoteRequest = (body) => {
    const issues = [];
    if (!isRecord(body)) {
        return {
            ok: false,
    ...
    if (!isVenue(body.venue)) {
        issues.push({ path: "$.venue", message: `must be one of: ${domain_1.VENUES.join(", ")}` });
    }
...
const emptyConnection = (timestamp) => ({
    configured: false,
    status: "disconnected",
    updatedAt: timestamp,
    maskedKeys: [],
});
const createApiRouter = (dependencies) => {
    ...
    const saveRuntime = (nextRuntime) => {
        dependencies.setRuntime(nextRuntime);
    };
    const mapStrategyErrorToStatus = (error) => {
        if (!(error instanceof runtime_1.StrategyRuntimeError)) {
            return {
                status: 500,
    ...
    const saveConnection = (venue, credentials) => {
    ...
    router.post("/connect/:venue", (req, res) => {
        ...
        if (venue === "save") {
            ...
            if (!isVenue(saveVenue)) {
                sendValidationError(res, [{ path: "$.venue", message: `must be one of: ${domain_1.VENUES.join(", ")}` }]);
                return;
            }
        ...
        if (!isVenue(venue)) {
            sendBadRequest(res, "INVALID_VENUE", `venue must be one of: ${domain_1.VENUES.join(", ")}`);
            return;
        }
        if (!isRecord(req.body)) {
            sendValidationError(res, [{ path: "$", message: "must be an object" }]);
    ...
    router.patch("/portfolio", (req, res) => {
        const validation = (0, domain_1.validatePortfolioUpdateRequest)(req.body);
    ...
    router.post("/orders/market", async (req, res) => {
        const validation = (0, domain_1.validateMarketOrderRequest)(req.body);
        if (!validation.success || !validation.data) {
            sendValidationError(res, validation.issues);
            return;
        }
    ...
    router.get("/metrics", (req, res) => {
        ...
        if (!analytics_1.METRICS_WINDOWS.includes(windowRaw)) {
            sendValidationError(res, [
                {
                    path: "$.window",
                    message: `must be one of: ${analytics_1.METRICS_WINDOWS.join(", ")}`,
                },
            ]);
            return;
        }
        ...
        if (!strategyId) {
            const account = (0, analytics_1.computeAccountAnalytics)(fills, balances.holdings, balances.equity, window);
            res.json({
                scope: "account",
                window,
                ...toResponse(account),
        ...
        const strategy = (0, analytics_1.computeStrategyAnalytics)(fills, balances.holdings, balances.equity, run, window);
        res.json({
            scope: "strategy",
            window,
            ...toResponse(strategy),
